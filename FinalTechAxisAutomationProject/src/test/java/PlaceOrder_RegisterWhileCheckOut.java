...
public class PlaceOrder_RegisterWhileCheckOut {
    WebDriver driver;
    ...
    public void registerWhileCheckout() throws InterruptedException {
        ...
        register.AddToCartproduct1();
        Thread.sleep(1000);

        register.Product1ViewCartBtn();
        ...
        register.ProceedToCheckoutCartPage();
        Thread.sleep(1000);

        register.ClickRegister_LoginBtn();
        ...
        //review order
        WebElement product = driver.findElement(By.xpath("//*[@id=\"product-1\"]/td[2]/p"));
        Boolean productAddedToCart = product.isDisplayed();
        Assert.assertTrue(productAddedToCart);

        register.EnterCommentText("This is a text message in comment about the order for QA testing.");
        register.ClickPlaceOrderBtn();
    }
     @Test(priority = 2)
...
//         Assert.assertEquals(verifyMessage,"Your order has been placed successfully!");
